// Бенчмарк: что дешевле — передавать/возвращать структуру по значению или по указателю.
// Размеры данных от 10 байт до 10 МБ. Вывод: на маленьких данных значение быстрее
// (нет аллокации в куче), выигрыш указателя начинается примерно от 1 МБ.
// Запуск: go test -bench=. ./05_pointers/pointer_perf_bench
...
func BenchmarkPointer10In(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		readMiddle10p(&v)
		// передача по указателю: копируются 8 байт адреса
	}
}
...
func BenchmarkPointer10Out(b *testing.B) {
	for i := 0; i < b.N; i++ {
		v := get10p()
		// возврат указателя обычно уводит данные в кучу (escape analysis)
		x = v.data[5]
	}
}
...
func BenchmarkValue10Out(b *testing.B) {
	for i := 0; i < b.N; i++ {
		v := get10v()
		// возврат значения на маленьких структурах дешевле: аллокации нет
		x = v.data[5]
	}
}
