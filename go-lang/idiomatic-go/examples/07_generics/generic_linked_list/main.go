// Обобщенный односвязный список с ограничением comparable (нужно для Index).
// Показывает, как параметр типа проходит через связанные структуры: Node[T] внутри List[T].
package main
...
type Node[T comparable] struct {
	// параметр типа проходит насквозь: Node[T] внутри List[T]
	Value T
	Next  *Node[T]
}
...
func (l *List[T]) Add(t T) {
	// приемник указательный — метод меняет список
	n := &Node[T]{
		Value: t,
	}
	if l.Head == nil {
