// Интерфейс с параметром типа как ограничение: Differ[T] требует Stringer + Diff(T).
// FindCloser[T Differ[T]] — «тип, умеющий сравнить себя с собой».
// Закомментированные вызовы показывают ошибки: разные типы в парах и тип без Diff.
package main
...
type Pair[T fmt.Stringer] struct {
	// ограничением может быть ЛЮБОЙ интерфейс, не только any/comparable
	Val1 T
	Val2 T
}
...
type Differ[T any] interface {
	fmt.Stringer
	Diff(T) float64
	// метод с параметром типа — «умею сравнить себя с таким же»
}

func FindCloser[T Differ[T]](pair1, pair2 Pair[T]) Pair[T] {
	// T Differ[T] — тип, который умеет сравнивать себя с собой
