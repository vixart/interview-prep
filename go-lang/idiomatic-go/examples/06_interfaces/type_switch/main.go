// Переключатель типа на практике: обход дерева выражения, где узел хранит
// маркерный интерфейс treeVal (число или оператор). Ветвь nil ловит пустой узел,
// default — тип, о котором функция еще не знает.
package main
...
func walkTree(t *treeNode) (int, error) {
	...
	// переключатель типа: в каждой ветви val уже нужного типа
	case nil:
		// отдельная ветвь для пустого интерфейсного значения
	...
	default:
		// страховка: добавили новый тип узла — попадем сюда, а не молча посчитаем неверно
