// Экзотика из главы про reflect: мемоизация ЛЮБОЙ функции.
// Ключ кэша — структура, СОЗДАННАЯ во время выполнения через reflect.StructOf
// из типов входных параметров, а сама обертка — через reflect.MakeFunc.
// Практическая ценность невелика, но видно, на что рефлексия способна в пределе.
package main
...
func buildInStruct(ft reflect.Type) (reflect.Type, error) {
	...
	for i := 0; i < ft.NumIn(); i++ {
		...
		if !ct.Comparable() {
			// ключ map обязан быть сравнимым — проверяем каждое поле будущей структуры
	...
	// тип структуры создан в рантайме — в исходниках его нет
	return s, nil
}
