// Бенчмарк трех реализаций Filter (рефлексия / дженерики / конкретный тип)
// плюс тесты, что все три дают одинаковый результат.
package filter
...
func BenchmarkFilterReflectString(b *testing.B) {
	// рефлексия: медленнее в разы и аллоцирует на каждом элементе
	vals := setup()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkFilterGenericString(b *testing.B) {
	// дженерик: почти как рукописный вариант ниже
	vals := setup()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
