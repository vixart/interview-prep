// Свой маршалер на рефлексии: структуры ↔ CSV по тегам `csv:"..."`.
// Показывает весь цикл — обход полей типа, чтение значений, создание среза
// через reflect.MakeSlice и запись значений через Set* (нужен указатель).
// Это и есть та работа, которую внутри делает encoding/json.
package main
...
func Marshal(v interface{}) ([][]string, error) {
	...
	if sliceVal.Kind() != reflect.Slice {
		// проверка Kind обязательна: вызов «не того» метода паникует
		return nil, errors.New("must be a slice of structs")
	}
	structType := sliceVal.Type().Elem()
	// Elem() дает тип ЭЛЕМЕНТА среза
...
func marshalHeader(vt reflect.Type) []string {
	...
	for i := 0; i < vt.NumField(); i++ {
		...
		if curTag, ok := field.Tag.Lookup("csv"); ok {
			// имя колонки берем из тега структуры
