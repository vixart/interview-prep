// Свой тип ошибки со статусом: StatusErr реализует error через метод Error().
// Возвращается он всегда как тип error — вызывающий не привязан к конкретному типу.
package main
...
func (se StatusErr) Error() string {
	// один метод Error() string — и тип уже является error
	return se.Message
}
...
func LoginAndGetData(uid, pwd, file string) ([]byte, error) {
	...
	if err != nil {
		return nil, StatusErr{
			// возвращаем свой тип, но объявленный тип результата — error
			Status:  InvalidLogin,
