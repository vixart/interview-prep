// t.Cleanup регистрирует уборку для КОНКРЕТНОГО теста — удобно во вспомогательных
// функциях, которые создают ресурсы: вызывающему тесту не нужно ничего помнить.
// Работает и при t.Fatal, и при панике, в отличие от кода в конце теста.
package cleanup
...
func createFile(t *testing.T) (_ string, err error) {
	...
	t.Cleanup(func() {
		// уборка привязана к тесту: сработает и при t.Fatal, и при панике
		os.Remove(f.Name())
	})
	return f.Name(), nil
}
