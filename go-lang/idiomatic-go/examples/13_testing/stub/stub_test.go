// Две техники заглушек:
//  1. GetPetNamesStub встраивает интерфейс Entities и переопределяет только нужный метод
//     (остальные есть формально, но паникуют при вызове — и это правильно);
//  2. EntitiesStub из полей-функций — поведение настраивается отдельно в каждом тесте.
package stub
...
type GetPetNamesStub struct {
	Entities
	// встроенный интерфейс: реализовывать нужно только используемые методы
}

func (ps GetPetNamesStub) GetPets(userID string) ([]Pet, error) {
	// переопределяем ровно один метод, который зовет тестируемый код
	switch userID {
	case "1":
