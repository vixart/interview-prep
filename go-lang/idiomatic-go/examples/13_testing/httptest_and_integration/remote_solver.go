// Реализация MathSolver поверх HTTP: именно ее тестируют через httptest-сервер
// (remote_solver_test.go) и через настоящий сервис (remote_solver_integration_test.go).
package solver
...
func (rs RemoteSolver) Resolve(ctx context.Context, expression string) (float64, error) {
	...
	// URL берется из поля — в тесте туда подставят адрес httptest-сервера
	if err != nil {
		return 0, err
	}
