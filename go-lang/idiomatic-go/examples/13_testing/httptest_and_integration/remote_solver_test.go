// httptest.NewServer поднимает НАСТОЯЩИЙ HTTP-сервер на случайном порту:
// клиент проверяется целиком (URL, кодирование параметров, разбор ответа),
// но внешний сервис не нужен. server.URL подставляется в тестируемый клиент.
package solver
...
func TestRemoteSolver_Resolve(t *testing.T) {
	...
	server := httptest.NewServer(
		// настоящий HTTP-сервер на случайном порту — внешний сервис не нужен
	...
	defer server.Close()
	// сервер обязательно останавливаем
	rs := RemoteSolver{
