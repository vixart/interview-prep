// Практичный паттерн: middleware, который кладет дедлайн в контекст КАЖДОГО запроса.
// Дальше обработчик обязан передавать ctx во все долгие операции и различать
// DeadlineExceeded (504) и прочие ошибки (500). Контекст сам ничего не прерывает —
// он только закрывает Done(), реагировать должен наш код.
// Запуск: go run . , затем curl -i localhost:8080/
package main
