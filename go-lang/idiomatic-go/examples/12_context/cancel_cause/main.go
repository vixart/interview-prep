// context.WithCancelCause: cancelFunc принимает ОШИБКУ-причину,
// которую потом можно достать через context.Cause(ctx).
// ctx.Err() при этом по-прежнему вернет обобщенное context.Canceled.
package main
...
func main() {
	...
	// то же самое, но отмена несет ПРИЧИНУ
	defer cancelFunc(nil)
	// nil означает «отмена без особой причины»
	ch := make(chan string)
	...
	go func() {
		...
		for {
			...
			if resp.StatusCode == http.StatusInternalServerError {
				...
				// причину потом достанем через context.Cause(ctx)
				return
			}
