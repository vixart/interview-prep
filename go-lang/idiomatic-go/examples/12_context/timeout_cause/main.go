// Комбинация таймаута и отмены с причиной: родительский контекст с WithTimeout,
// дочерний с WithCancelCause. Кто сработал первым — таймаут или ошибка —
// видно по context.Cause(ctx).
package main
...
func main() {
	...
	// верхняя граница по времени на всю работу
	defer cancelFuncParent()
	ctx, cancelFunc := context.WithCancelCause(ctx)
	// поверх таймаута — отмена с причиной; побеждает то, что сработает раньше
