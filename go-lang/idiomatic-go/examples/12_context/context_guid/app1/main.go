// Первый сервис цепочки: получает запрос, логирует с GUID из контекста
// и делает запрос ко второму сервису, прокидывая GUID дальше.
// Запуск: go run . (порт 8080), второй сервис — go run ../app2 (порт 8081)
package main
...
func (c Controller) First(rw http.ResponseWriter, req *http.Request) {
	// контекст берем из запроса и дальше передаем ЯВНО первым параметром
	ctx := req.Context()
