// Этап C конвейера: принимает результат этапа AB и так же завершается
// по первому из трех событий — результат, ошибка или отмена контекста.
package main
...
func (p *cProcessor) wait(ctx context.Context) (COut, error) {
	select {
	// три исхода: результат, ошибка или отмена контекста
	case out := <-p.outC:
		return out, nil
	case err := <-p.errs:
		return COut{}, err
